...
# The real summary is generated by the worker (Phase 4 will
# add a richer protocol: the worker will append a brief
# human-readable summary after `<task:done/>`, which the
# orchestrator will pick up). For now we just emit a fixed
        ...
        try:
            async for line in self.worker.read_lines():
                ...
                if TASK_DONE_MARKER in line:
                    ...
                    await self._broadcast(
                        {
                            ...
                            "summary": _DEFAULT_COMPLETE_SUMMARY,
                        }
                    )
                    await self._broadcast(
                    ...
                    continue
                await self._broadcast(
                    {"type": WS_TYPE_WORKER_LINE, "line": line}
                )
        except asyncio.CancelledError:
            raise
        except Exception as exc:
